<section class="space-y-5">
    ...
    <div class="grid gap-3 sm:grid-cols-3">
        <label class="grid gap-1 text-sm font-semibold" for="search">Search<input id="search" wire:model.live.debounce.300ms="search" x-bind:disabled="$store.connectivity.offline" class="min-h-11 rounded-md border border-slate-300 bg-white px-3 focus-visible:outline-2 focus-visible:outline-sky-700" placeholder="Title or speaker"></label>
        <label class="grid gap-1 text-sm font-semibold" for="browser-room">Room<select id="browser-room" wire:model.live="roomId" x-bind:disabled="$store.connectivity.offline" class="min-h-11 rounded-md border border-slate-300 bg-white px-3"><option value="">All rooms</option>@foreach ($rooms as $room)<option value="{{ $room->id }}">{{ $room->name }}</option>@endforeach</select></label>
        <label class="grid gap-1 text-sm font-semibold" for="browser-date">Date<select id="browser-date" wire:model.live="date" x-bind:disabled="$store.connectivity.offline" class="min-h-11 rounded-md border border-slate-300 bg-white px-3"><option value="">All dates</option>@foreach ($dates as $scheduleDate)<option value="{{ $scheduleDate }}">{{ \Carbon\CarbonImmutable::parse($scheduleDate)->isoFormat('ddd D MMM') }}</option>@endforeach</select></label>
    </div>
    <div class="flex flex-wrap gap-3"><label class="flex min-h-11 items-center gap-3 rounded-md border border-slate-300 bg-white px-3 text-sm font-semibold"><input wire:model.live="mySessions" x-bind:disabled="$store.connectivity.offline" type="checkbox" class="size-5 accent-sky-700"> My sessions only</label>@if ($currentUser->is_admin)<label class="flex min-h-11 items-center gap-3 rounded-md border border-slate-300 bg-white px-3 text-sm font-semibold"><input wire:model.live="showRemoved" x-bind:disabled="$store.connectivity.offline" type="checkbox" class="size-5 accent-sky-700"> Show removed sessions</label>@endif @if ($search !== '' || $roomId !== null || $date !== null || $showRemoved || $mySessions)<button wire:click="clearFilters" x-bind:disabled="$store.connectivity.offline" type="button" class="min-h-11 rounded-md border border-slate-300 bg-white px-4 text-sm font-semibold text-slate-700 disabled:cursor-not-allowed disabled:text-slate-400 focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-sky-700">Clear filters</button>@endif</div>
    <div class="space-y-3">
